...
/*
 * Flush stats to Google Stackdriver Monitoring API (https://cloud.google.com/monitoring).
 *
 ...
 *
 *	credentialsPath: Your Stackdriver service account key path, generated in your account settings
 *  source: The instance ID of the GCP machine that the values should be associated with (optional)
...
// them out
const internalMetricsPrefix = /^statsd\./;
...
function StackdriverBackend(startupTime, config, emitter) {
	var self = this;
	this.credentialsPath = ('credentialsPath' in config.stackdriver) ? config.stackdriver.credentialsPath : process.env.GOOGLE_APPLICATION_CREDENTIALS;
	...
	if (this.debug) {
		console.log('Stackdriver Backend set to report timer percentiles ' + this.percentileValues);
	}
	...
	if (!this.credentialsPath) {
		console.error("ERROR: no api key set, all flush operations will no-op!");
	}

	if (this.debug) {
		console.log('Stackdriver backend is running in debug mode, extra logging will occur');
	}

	if (this.sourceFromPrefix) {
		console.log('Source will be inferred from each metric prefix, separated by "' + this.sourcePrefixSeparator + '", all points sent to Stackdriver for that metric will be associated with that instance');
	} else if (this.source) {
		var sys = require('sys');
	 	...
	 	if (this.source == 'detect-aws') {
		 	...
		 	var child = exec('wget -q -O - http://169.254.169.254/latest/meta-data/instance-id', function(error, stdout, stdin) {
		 		self.source = stdout;
		 		console.log('Auto-detected aws instance id as ' + self.source);
		  	});
	 		...
	 		var child = exec('wget -q -O - http://metadata/computeMetadata/v1/instance/id --header "X-Google-Metadata-Request: true"', function(error, stdout, stdin) {
		 		self.source = stdout;
		 		console.log('Auto-detected gce instance id as ' + self.source);
		  	});	
	 	}

		console.log('Source param set to ' + this.source + ', all points sent to Stackdriver will be associated with that instance');
	}
	// attach
...
StackdriverBackend.prototype.add_point_to_message = function(stackdriverMessage, point) {
	...
	if (this.sourceFromPrefix) {
		...
		if (sep <= 0) {
			if (this.debug) {
				console.log('No prefix separator ("' + this.sourcePrefixSeparator + '") found in metric name, ' + point.name + ' will be sent as a regular custom metric not attached to an instance');
			...
			if (this.debug) {
				console.log('Prefix separator ("' + this.sourcePrefixSeparator + '") found in metric name, ' + point.name + ' will be used as the metric name, and attached to instance id ' + point.instance);
			}
		}
	} else if (this.source) {
	...
	var proto_formatted_point = {
		interval: {
			endTime: {
				seconds: point.collected_at,
			}
		},
		value: {
	...
	stackdriverMessage.push(proto_formatted_point);
}
...
StackdriverBackend.prototype.post_message_to_gateway = function(stackdriverMessage) {
	...
	if (!this.credentialsPath) {
		console.error("No API key set, Stackdriver API key must be set before posting data");
		return;
	}
	
	if (!stackdriverMessage) {
		console.error("No message to send.  Need a Stackdriver Custom Metrics JSON message");
		return;
	}

	if (this.debug) {
		console.log("Message contents to be posted:");
		console.log(JSON.stringify(stackdriverMessage));
	}
	...
	const request = {
		...
		timeSeries: [
			...
			metric: {
				type: 'custom.googleapis.com/airflow/statsd',
				labels: {},
			},
			resource: {
				type: 'global',
			...
			points: [stackdriverMessage[0]],
			},
		],
	};
	
	// Writes time series data
	const result = client.createTimeSeries(request,
		function(err, data){
			if (err) {
				// log an error
				console.error(err);
				...
				// Success
				console.log(`Done writing time series data.`, data);
			  }
		});
...
StackdriverBackend.prototype.flush = function(timestamp, metrics) {
	...
	if (this.sendCounters) {
		for (counter_key in metrics.counters) {
			if (counter_key.match(internalMetricsPrefix) != null) {
				if (this.debug) {
					console.log("Skipping internal metric " + counter_key);
				}
				continue;
			} else {
				if (metrics.counters[counter_key] == 0 && !this.sendZeroTimersAndRates) {
				  if (this.debug) {
				    console.log("Skipping zero value counter " + counter_key)
				  }
				  continue
				} else if (this.debug) {
          console.log("Found counter " + counter_key);
        }
	...
	if (this.sendCounterRates) {
		for (counter_key in metrics.counter_rates) {
          ...
          if (this.debug) {
            console.log("Skipping zero value counter rate " + counter_key)
          }
          continue
        } else if (this.debug) {
					console.log("Found counter rate " + counter_key);
				}
				this.add_point_to_message(stackdriverMessage, {
	...
	if (this.sendGauges) {
		for (gauge_key in metrics.gauges) {
			if (gauge_key.match(internalMetricsPrefix) != null) {
				if (this.debug) {
					console.log("Skipping internal metric " + gauge_key);
				}
				continue;
			} else {
				if (this.debug) {
					console.log("Found gauge " + gauge_key);
				}
				this.add_point_to_message(stackdriverMessage, {
	...
	for (timer_key in metrics.timer_data) {
		if (timer_key.match(internalMetricsPrefix) != null) {
			if (this.debug) {
				console.log("Skipping internal metric " + timer_key);
			}
			continue;
		} else {
			if (this.debug) {
				console.log("Found timer " + timer_key);
			}
          ...
          if (this.debug) {
            console.log("Skipping zero value timer count " + timer_key)
          }
        } else {
  				this.add_point_to_message(stackdriverMessage, {
          ...
          if (this.debug) {
            console.log("Skipping zero value timer rate " + timer_key)
          }
        } else {
  				this.add_point_to_message(stackdriverMessage, {
  					name : timer_key + ".rate",
	...
	if (this.sendSets) {
		for (set_key in metrics.sets) {
			if (set_key.match(internalMetricsPrefix) != null) {
				if (this.debug) {
					console.log("Skipping internal metric " + set_key);
				}
				continue;
			} else {
				if (this.debug) {
					console.log("Found set " + set_key);
				}
				this.add_point_to_message(stackdriverMessage, {
					name : set_key + ".count",
	...
	if (stackdriverMessage.length > 0) {
		console.log(stackdriverMessage.length
				+ " metrics found, posting to Stackdriver");
		this.post_message_to_gateway(stackdriverMessage);
	} else {
		if (this.debug) {
			console.log("No points to send");
		}
	}
